...
The package also contains `audit_Final_Validation_Shield.ipynb`, retained as an audit/provenance record rather than as the canonical source-to-result execution path, and `06_Experiment_06_Evidence_Record.md`, which records the exact locked design, numerical outputs, alignment-control result, and claim boundary for the compact-representation experiment. Fresh executable reproducibility work is additionally deposited under `Codes_paper_I/Experiment_08/`, with its scientific status and complete evidence chronology recorded in `docs/experiment-08/EXPERIMENT08_EVIDENCE_PROVENANCE_MANIFEST.md`. Experiment 08 failed its frozen mechanical gate and all subsequent predictive results are explicitly classified as post-outcome / exploratory.

In addition to the executable Paper-I notebooks and the Experiment 06 provenance record, the repository contains reviewer-facing numerical evidence under `papers/CLO-SKET/evidence/`. The bundle contains frozen Experiment 06 and Experiment 07 CSV/JSON evidence, manifests, provenance hashes, fold-level summaries, and the out-of-fold/identity-bootstrap records required to audit the manuscript-facing numerical claims, together with deposited Experiment-08 evidence under `evidence/Experiment_08/`. Experiment 08 remains governed by its failed frozen mechanical gate, and all subsequent predictive results are post-outcome / exploratory. `evidence/PUBLIC_EVIDENCE_MANIFEST.json` records the included public evidence files, byte sizes, and SHA-256 hashes.

The public evidence bundle is intended for numerical audit and provenance verification; it is not presented as a self-contained replacement for the original CLO-SKET dataset or every frozen computational intermediate. In particular, the historical Experiment 06 runtime checkpoint and the 2,300 × 8,100 Experiment 07 HOG feature matrix are intentionally excluded from Git. The latter is a deterministic intermediate generated by the public Experiment 07 extraction code. Large historical runtime-memory snapshots are likewise not treated as the scientific source of record.
...
The reproducibility package is intended to regenerate and audit the Paper-I measurement and validation chain without requiring Paper-II-only code. Historical `.pkl` checkpoint references that survive inside audit/recovery cells do not redefine the scientific source of record. The canonical public evidence comprises the explicit source code, the original CLO-SKET images obtained from their official source, the final manuscript Methods/Results, the Experiment 06 provenance record, the reviewer evidence under `evidence/` including `evidence/Experiment_08/`, and the Paper-I/Paper-II claim firewall. The bundle supports numerical audit of the frozen Experiment 06 and Experiment 07 manuscript results and the separately status-bounded Experiment-08 record without implying that every large computational intermediate is redistributed through Git.
