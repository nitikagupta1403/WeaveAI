...
In addition to the executable Paper-I notebooks and the Experiment 06 provenance record, the repository contains a compact reviewer-facing numerical evidence bundle under `papers/CLO-SKET/evidence/`. The bundle contains frozen Experiment 06 and Experiment 07 CSV/JSON evidence, manifests, provenance hashes, fold-level summaries, and the out-of-fold/identity-bootstrap records required to audit the manuscript-facing numerical claims. `evidence/PUBLIC_EVIDENCE_MANIFEST.json` records the included files, byte sizes, and SHA-256 hashes.
...
The reproducibility package is intended to regenerate and audit the Paper-I measurement and validation chain without requiring Paper-II-only code. Historical `.pkl` checkpoint references that survive inside audit/recovery cells do not redefine the scientific source of record. The canonical public evidence comprises the explicit source code, the original CLO-SKET images obtained from their official source, the final manuscript Methods/Results, the Experiment 06 provenance record, the compact reviewer evidence bundle under `evidence/`, and the Paper-I/Paper-II claim firewall. The bundle supports numerical audit of the frozen Experiment 06 and Experiment 07 manuscript results without implying that every large computational intermediate is redistributed through Git.
